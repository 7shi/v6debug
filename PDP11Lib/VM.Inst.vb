...
Partial Public Class VM
    Public Sub RunStep()
        ...
        Select Case Me(PC + 1) >> 4
            ...
            Case 1 ' mov: MOVe
                Dim oprs = GetSrcDst()
                Dim src = oprs.Item1.GetValue(Me)
                oprs.Item2.SetValue(Me, src)
                SetFlags(src = 0, ConvShort(src) < 0, C, False)
                Return
            Case 2 ' cmp: CoMPare
                Dim oprs = GetSrcSrc()
                Dim src = oprs.Item1.GetValue(Me)
                Dim dst = oprs.Item2.GetValue(Me)
                Dim val = CInt(ConvShort(src)) - CInt(ConvShort(dst))
                SetFlags(val = 0, val < 0, src < dst, val < -&H8000)
                Return
            Case 3 ' bit: BIt Test
                Dim oprs = GetSrcSrc()
                Dim val = oprs.Item1.GetValue(Me) And oprs.Item2.GetValue(Me)
                SetFlags(val = 0, (val And &H8000) <> 0, C, False)
                Return
            Case 4 ' bic: BIt Clear
                Dim oprs = GetSrcDst()
                Dim val = (Not oprs.Item1.GetValue(Me)) And oprs.Item2.GetValue(Me)
                oprs.Item2.SetValue(Me, val)
                SetFlags(val = 0, (val And &H8000) <> 0, C, False)
                Return
            Case 5 ' bis: BIt Set
                Dim oprs = GetSrcDst()
                Dim val = oprs.Item1.GetValue(Me) Or oprs.Item2.GetValue(Me)
                oprs.Item2.SetValue(Me, val)
                SetFlags(val = 0, (val And &H8000) <> 0, C, False)
                Return
            Case 6 ' add: ADD
                ...
                Dim dst = oprs.Item2.GetValue(Me)
                Dim val = CInt(ConvShort(src)) + CInt(ConvShort(dst))
                oprs.Item2.SetValue(Me, CUShort(val And &HFFFF))
            ...
            Case &O11 ' movb: MOVe Byte
                Dim oprs = GetSrcDst()
                Dim src = oprs.Item1.GetByte(Me)
                oprs.Item2.SetByte(Me, src)
                SetFlags(src = 0, ConvSByte(src) < 0, C, False)
                Return
            Case &O12 ' cmpb: CoMPare Byte
                Dim oprs = GetSrcSrc()
                Dim src = oprs.Item1.GetByte(Me)
                Dim dst = oprs.Item2.GetByte(Me)
                Dim val = CInt(ConvSByte(src)) - CInt(ConvSByte(dst))
                SetFlags(val = 0, val < 0, src < dst, val < -&H80)
                Return
            Case &O13 ' bitb: BIt Test Byte
                Dim oprs = GetSrcSrc()
                Dim val = oprs.Item1.GetByte(Me) And oprs.Item2.GetByte(Me)
                SetFlags(val = 0, (val And &H80) <> 0, C, False)
                Return
            Case &O14 ' bicb: BIt Clear Byte
                Dim oprs = GetSrcDst()
                Dim val = (Not oprs.Item1.GetByte(Me)) And oprs.Item2.GetByte(Me)
                oprs.Item2.SetByte(Me, val)
                SetFlags(val = 0, (val And &H80) <> 0, C, False)
                Return
            Case &O15 ' bisb: BIt Set Byte
                Dim oprs = GetSrcDst()
                Dim val = oprs.Item1.GetByte(Me) Or oprs.Item2.GetByte(Me)
                oprs.Item2.SetByte(Me, val)
                SetFlags(val = 0, (val And &H80) <> 0, C, False)
                Return
            ...
            Case 5
                Select Case v2
                    ...
                    Case 6 ' sbc: SuBtract Carry
                        ...
                        Return
                    Case 7 ' tst: TeST
                        Dim dst = ConvShort(GetSrc.GetValue(Me))
            ...
            Case 6
                Select Case v2
                    ...
                    Case 7 ' sxt: Sign eXTend
                        GetDst.SetValue(Me, If(N, &HFFFFUS, 0US))
    ...
    Private Sub Exec7()
        ...
        Select Case (v >> 9) And 7
            ...
            '    End Select
            Case 0 ' mul: MULtiply
                Dim src = ConvShort(GetSrc.GetValue(Me))
                Dim r = (v >> 6) And 7
            ...
            Case 2 ' ash: Arithmetic SHift
                Dim src = GetSrc.GetValue(Me) And &O77
                Dim r = (v >> 6) And 7
    ...
    Private Sub Exec10()
        ...
        Select Case (v >> 6) And &O77
            ...
            Case &O57 ' tstb: TeST Byte
                Dim dst = ConvSByte(GetSrc.GetByte(Me))
                SetFlags(dst = 0, dst < 0, False, False)
                Return
